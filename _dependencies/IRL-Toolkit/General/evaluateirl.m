...
for m=1:length(test_models)
    ...
    for k=1:length(test_metrics)
        ...
        metric_scores{m,k} = feval(strcat(cur_metric,'score'), mdp_soln, true_r, irl_soln, irl_r, feature_data, true_feature_map, mdp_data, mdp_params, cur_model);
    end
end
...
test_result = struct(...
    ...
    'mdp',mdp);
    %'sparsity', irl_result.sparsity); This broke the test so I took it out for now -- Mark 11/24/2017